...
test_that("scICE_clustering verbose emits startup, checkpoint, and completion logs", {
  ...
  expect_true(any(grepl("Preliminary trial strategy - reuse one representative preliminary clustering per gamma step", captured$messages, fixed = TRUE)))
  expect_true(any(grepl("lower-bound progress", captured$messages, fixed = TRUE)))
  expect_true(any(grepl("trial summary - full preliminary trial sets:", captured$messages, fixed = TRUE)))
