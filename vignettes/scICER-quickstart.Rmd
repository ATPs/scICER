...
scice_results <- scICE_clustering(
  ...
  min_cluster_size = 5,          # Merge clusters smaller than 5 cells
  verbose = TRUE
)
...
if (length(scice_results$consistent_clusters) > 0) {
  ...
  if (!is.null(cluster_labels)) {
    ...
    first_clustering <- names(cluster_labels)[2]  # Skip cell_id column
    cat("\nFirst 10 assignments for", first_clustering, ":\n")
    print(head(cluster_labels[, c("cell_id", first_clustering)], 10))
  }
} else {
...
- **min_cluster_size**: 5 by default; set 2 to mimic singleton handling, set 1 to disable merging
